...
name: improve-react
description: >-
  Survey a React codebase as a senior React engineer using React Doctor scan
  evidence, then write prioritized self-contained plans (read-only on source).
  Use when the user asks to improve React code, audit the React codebase, make
  the app faster or more robust, or wants a roadmap of React fixes. For
  regression check or fix-now triage use react-doctor. Not for visual taste
  or browser automation (playwright).
---
