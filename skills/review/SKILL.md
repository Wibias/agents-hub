...
name: review
description: >-
  Runs a two-axis diff review — Standards (coding conventions, ADRs) and Spec (does the diff match the originating issue/PRD?) — across changes since a user-supplied fixed point. Use when the user asks to "review since X", "review this branch/PR", or "review work-in-progress". Not an over-engineering filter (use ponytail-review).
---
...
On top of whatever the repo documents, the Standards axis always carries the **smell baseline** in [references/code-smells.md](references/code-smells.md) -- 12 Fowler code smells (_Refactoring_, ch.3) that apply even when a repo documents nothing. Two rules bind it: the repo overrides (a documented repo standard always wins; where it endorses something the baseline would flag, suppress the smell); and it is always a judgement call (each smell is a labelled heuristic, never a hard violation -- skip anything tooling already enforces). Paste the smell table into the Standards sub-agent prompt so it has access.
...
Integration source: Martin Fowler's code-smell catalog via [mattpocock/skills](https://github.com/mattpocock/skills) (MIT). Adapted 2026-07-10.
