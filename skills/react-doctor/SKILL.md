...
```powershell
# Windows hub
(Invoke-WebRequest -Uri "https://www.react.doctor/prompts/react-doctor-agent.md" -Headers @{ "Cache-Control" = "no-cache" } -UseBasicParsing).Content
# or: curl.exe --fail --silent --show-error --header "Cache-Control: no-cache" https://www.react.doctor/prompts/react-doctor-agent.md
```

The playbook is the single source of truth - scan → filter → triage → fix → validate (edits working tree; never commits; never opens PRs).
...
- Roadmap / plans only (no source edits): skill **`improve-react`**
- Pre-merge orchestration with VibeDoctor: skill **`vibedoctor-scan`**
