...
name: handoff
description: >
  Compacts the current conversation into a structured handoff document so a fresh
  agent or session can continue the work without the original chat history. Captures
  current goal, completed steps, open decisions, files touched, known failures, next
  concrete steps, and a suggested-skills section. Saves the document to the OS temp
  directory, not the workspace. Use when the user says "create a handoff", "hand
  this off", "hitting context limits", "switching focus", or before archiving a long
  session. Does not duplicate content already in PRDs, plans, ADRs, issues, or
  commits -- references those by path or URL instead.
---
...
Include this at the end of the handoff (inside the code block):
...
Add a "Suggested skills" section before the fresh agent prompt, naming skills that would help the next session:

```
## Suggested skills
- $skill-name -- reason this would help
```
...
- **Windows:** `$env:TEMP\handoff-<random-8-chars>.md`
- **macOS/Linux:** `$TMPDIR/handoff-<random-8-chars>.md`
- If the user prefers an in-repo record, save to `HANDOFF.md` in the project root instead.

After saving, tell the user the absolute path. The user can then start a fresh session with:

```
Read the file <absolute-path> to get the context, then wait for instructions.
```

Do not duplicate content already in PRDs, plans, ADRs, issues, commits, or diffs -- reference those by path or URL instead.

---
Source: davidondrej/skills (https://github.com/davidondrej/skills), MIT License. Adapted 2026-07-10.
